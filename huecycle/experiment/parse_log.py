...
with open("log.txt") as f:
    for line in f:
        ...
        time = datetime.datetime.fromisoformat(
            d["light"]["light_level_report"]["changed"]
        )
        times.append(time)
        ...
        weights = [
            (tb - ta).total_seconds() / (5 * 60)
            for (ta, tb) in itertools.pairwise(times)
        ]
        z = zip(levels, times)
        #            delta v    delta t                    normally
        levels_dt = [
            (a) * (tb - ta).total_seconds() / (5 * 60)
            for ((a, ta), (b, tb)) in itertools.pairwise(z)
        ]
