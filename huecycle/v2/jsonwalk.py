...
def trace(locals_from_stack):
    return ''.join(f"\n{n*'-'}> {p}" for n, p in enumerate(
        lokals['__key__'] for lokals in locals_from_stack))

def locals_with_key():
    return [fl for fl in
                (tb.tb_frame.f_locals for tb in iterate(attrgetter('tb_next'), sys.exc_info()[2]))
                if '__key__' in fl]
    ...
    def walk_fn(subject, accu=None, **opts):
        ...
        try:
            ...
            if catch:
                ...
                raise Exception(f'{e.__class__.__name__}: {str(e)} at:{trace(locals_from_stack)} while processing:\n{pformat(subject)}') from e
            raise e
    return walk_fn
...
def ignore_silently(a, *_, **__): return a
def unsupported(_,__,p,___): raise Exception(f"Unsupported predicate '{p}'")
    ...
    w = walk({
        '__switch__': lambda a,s: s['base'][0]['@value'],
         2: {
                ...
                'x': lambda a,s,p,os: a | {'result': [{'@value': 2**o['@value']} for o in os]},
         },
         8: {
                'base': ignore_silently,
                'x': lambda a,s,p,os: a | {'result': [{'@value': 8**o['@value']} for o in os]},
         }
    })
    r = w({'base': [{'@value': 2}],
              'x': [{'@value': 3}]})
...
def subwalk_from_mods():
    w = walk({
        '__switch__': lambda a,s: s['@type'][0],
        'personal': {
            '@type': lambda a,s,p,os: a | {'type is': os}
        }
    })
    r = w({
        '@type': ['personal'],
...
def builtin_subwalk_stack_for_errors():
    ...
    with test.raises(Exception, "LookupError: No rule for 'e' in {} at:\n> b\n-> d\n--> e while processing:\n{'e': 42}"):
        w({'b': [{'d': [{'e': 42}]}]})
    with test.raises(Exception, "LookupError: No rule for 'X' in {'b'} at:\n> a\n-> X while processing:\n{'X': [{'e': 42}]}"):
        w({'a': [{'X': [{'e': 42}]}]})
...
more_rules = {
    ...
    dcterms+'name'     : lambda a,s,p,os: a | (lambda name=os[0]['@value'].split():
    ...
    foaf+'familyName'  : lambda a,s,p,os: a | {schema+'name':
        [{'@value': s.get(foaf+'givenName')[0]['@value'] + ' ' + os[0]['@value']}]},
    foaf+'givenName'   : ignore_silently,
    dcterms+'publisher': lambda a,s,p,os: a | {schema+'publisher': [{schema+'name': [o]} if '@value' in o else walk_one(o) for o in os]},
...
def keep_first_id():
    ...
    m = walk_all([{'@id': 'first'}, {'@id': 'second', schema+'about': [{'@id': 'third'}]}])
...
@test
def recur():
    m = walk_one({dcterms+'publisher': [{foaf+'name': [{'@value': 'Aap noot mies'}]}]})
    test.eq({schema+'publisher': [{schema+'name': [{'@value': 'Aap noot mies'}]}]}, m)
...
def combine_properties():
    object = expand({'dcterms:creator': {'foaf:givenName': ['Voornaam'], 'foaf:familyName': 'Achternaam'}})
    result = walk_one(object[0])
    test.eq({'http://purl.org/dc/terms/creator': [{'http://schema.org/name': [{'@value': 'Voornaam Achternaam'}]}]}, result)


@test
def split_properties():
    result = walk_one({dcterms+'creator': [{dcterms+'name': [{'@value': 'Voornaam Achternaam'}]}]})
    test.eq({dcterms+'creator': [{foaf+'givenName': [{'@value': 'Voornaam'}], foaf+'familyName': [{'@value': 'Achternaam'}]}]}, result)
...
def test_flatten_expanded_round_trip():
    ...
    flt = jsonld.flatten(doc, {}) # this does not work => {'@graph': {'@container': '@id'}})
    ...
    test.eq({
        '_:b0': {'@id': '_:b0', 'http://purl.org/dc/terms/creator': [{'@id': '_:b1'}]},
        '_:b1': {'@id': '_:b1', 'http://xmlns.com/foaf/0.1/name': [{'@value': 'Piet Pietersen'}]}},
        node_index(flt_exp))
