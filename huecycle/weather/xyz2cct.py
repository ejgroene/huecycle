...
# In Python, this becomes a tuple: (u, v, t)
...
def XYZtoCCT(xyz):
        ...
        for i in range(0, 31):
            ...
            dm = di
        if (i == 31):
            raise "WTF?" # oplossen with if-else?
        di = di / sqrt(1.0 + uvt[i    ][2] * uvt[i    ][2])
        dm = dm / sqrt(1.0 + uvt[i - 1][2] * uvt[i - 1][2])
        p = dm / (dm - di) # /* p = interpolation parameter, 0.0 : i-1, 1.0 : i */
        p = 1.0 / (LERP(rt[i - 1], rt[i], p))
        return int(p)
